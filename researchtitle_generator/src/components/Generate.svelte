<script>
    ...
    let researchTitle = "Click the button to generate a research title";

  // Function to get a random title from the array
  function getRandomTitle() {
    const randomIndex = Math.floor(Math.random() * researchTitles.length);
    return researchTitles[randomIndex];
  }

  // Function to update the research title output field
  function updateResearchTitle() {
    researchTitle = getRandomTitle();
  }
</script>

<div class="flex justify-center flex-col items-center" >
  <p id="researchTitle" class="font-bold dark:text-purple-300 text-purple-500 text-base lg:text-lg text-center">"{researchTitle}"</p>
  <button on:click={updateResearchTitle} class="bg-purple-500 text-white p-3 rounded-full font-bold mt-10 dark:hover:bg-white dark:hover:text-purple-400 hover:text-purple-400 hover:bg-gray-800">Generate 🚀</button>
</div>